...
function show_logo() {
    ...
    echo -e "\033[0m"
    echo ""
    echo -e "\033[1;36mVersion:\033[0m \033[33m5.1.5\033[0m"
...
function install_bot() {
    ...
    DEBIAN_FRONTEND=noninteractive sudo apt install -y php8.3 php8.3-fpm php8.3-mysql || {
        echo -e "\e[91mError: Failed to install PHP 8.3 and related packages.\033[0m"
        exit 1
    }
...
# Add Apache configuration for 
echo "Adding Apache configuration for ..."
...
if ! grep -q "Alias / /www/html/" "$APACHE_CONF"; then
...
while [[ ! "$domainname" =~ ^[a-zA-Z0-9.-]+$ ]]; do
    ...
    if [ $? -eq 0 ]; then
        ...
        if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'mirzabot'") ]]; then
            ...
            echo -e "<?php" >> /www/html/config.php
            echo -e "${ASAS}APIKEY = '${YOUR_BOT_TOKEN}';" >> /www/html/config.php
            echo -e "${ASAS}usernamedb = '${dbuser}';" >> /www/html/config.php
            echo -e "${ASAS}passworddb = '${dbpass}';" >> /www/html/config.php
            echo -e "${ASAS}dbname = '${dbname}';" >> /www/html/config.php
            echo -e "${ASAS}domainhosts = '${YOUR_DOMAIN}';" >> /www/html/config.php
            echo -e "${ASAS}adminnumber = '${YOUR_CHAT_ID}';" >> /www/html/config.php
            echo -e "${ASAS}usernamebot = '${YOUR_BOTNAME}';" >> /www/html/config.php
            echo -e "${ASAS}secrettoken = '${secrettoken}';" >> /www/html/config.php
            echo -e "${ASAS}connect = mysqli_connect('localhost', \$usernamedb, \$passworddb, \$dbname);" >> /www/html/config.php
            echo -e "if (${ASAS}connect->connect_error) {" >> /www/html/config.php
            echo -e "die(' The connection to the database failed:' . ${ASAS}connect->connect_error);" >> /www/html/config.php
            echo -e "}" >> /www/html/config.php
            echo -e "mysqli_set_charset(${ASAS}connect, 'utf8mb4');" >> /www/html/config.php
...
echo -e "$text_to_save" >> /www/html/config.php
            echo -e "?>" >> /www/html/config.php
...
function install_bot_with_marzban() {
    ...
    # Add Ondřej Surý PPA for PHP 8.3
    ...
    # Install PHP 8.3 and all necessary modules (including PDO)
    DEBIAN_FRONTEND=noninteractive sudo apt install -y php8.3 php8.3-fpm php8.3-mysql php8.3-mbstring php8.3-zip php8.3-gd php8.3-curl php8.3-soap php8.3-ssh2 libssh2-1-dev libssh2-1 php8.3-pdo || {
        echo -e "\e[91mError: Failed to install PHP 8.3 and modules.\033[0m"
        exit 1
    }

    # Install additional Apache module
    sudo apt install -y libapache2-mod-php8.3 || {
        echo -e "\e[91mError: Failed to install libapache2-mod-php8.3.\033[0m"
        exit 1
    }
...
function update_bot() {
    ...
    # Backup config file
    CONFIG_PATH="/www/html/config.php"
    ...
    sudo mv "$EXTRACTED_DIR"/* /www/html/ || {
    ...
    if [ -f "/www/html/install.sh" ]; then
        sudo cp /www/html/install.sh /root/install.sh
        echo -e "\n\e[92mCopied latest install.sh to /root/install.sh.\033[0m"
    else
        echo -e "\n\e[91mWarning: install.sh not found in /www/html/ after update. Cannot update /root/install.sh.\033[0m"
    fi
...
function remove_bot() {
    ...
    BOT_DIR="/www/html"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m Mirza Bot is not installed (/www/html not found)." | tee -a "$LOG_FILE"
    ...
    sudo apt-get purge -y mysql-client-core-8.0 mysql-server-core-8.0 mysql-common php-mysql php8.3-mysql php8.3-mysql php-mariadb-mysql-kbs
...
function manage_additional_bots() {
    ...
    if [ ! -d "/www/html" ]; then
        echo -e "\033[31m[ERROR]\033[0m The main Mirza Bot is not installed (/www/html not found)."
...
function change_domain() {
    ...
    CONFIG_FILE="/www/html/config.php"
...
<VirtualHost *:443>
    ServerName $DOMAIN_NAME
    DocumentRoot /www/html$BOT_NAME
...
function update_additional_bot() {
    ...
    # List directories in /www/html excluding
    BOT_DIRS=$(ls -d /var/www/html/*/ 2>/dev/null | grep -v "/www/html" | xargs -n 1 basename)
...
function export_additional_bot_database() {
    ...
    # List all directories in /www/html excluding 
    BOT_DIRS=$(ls -d /var/www/html/*/ 2>/dev/null | grep -v "/www/html" | xargs -n 1 basename)
