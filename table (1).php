...
try {
    ...
    // استفاده از $connect به جای $pdo
    $stmt = $connect->prepare("SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?");
    $stmt->bind_param('s', $tableName);
    $stmt->execute();
    $result = $stmt->get_result();
    $tableExists = $result->fetch_assoc();
    ...
    if (!$tableExists) {
        ...
        if ($connect->query($createTableQuery)) {
            echo "جدول user با موفقیت ایجاد شد\n";
        } else {
            throw new Exception("خطا در ایجاد جدول user: " . $connect->error);
        }
    } else {
        echo "جدول user از قبل وجود دارد\n";
    ...
    file_put_contents('error_log_user.txt', $e->getMessage() . "\n", FILE_APPEND);
    echo "خطا در ایجاد جدول user: " . $e->getMessage() . "\n";
}
